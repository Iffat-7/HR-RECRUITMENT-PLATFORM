...
export default function Login() {
  const { user, isHr, initializing } = useAuth();
  ...
  const onGoogleSignIn = async () => {
    ...
    try {
      ...
      if (error) throw error;
      // Browser redirects to Google; on return, onAuthStateChange fires and
      // the redirect effect above sends the user to /admin or /candidate.
    } catch (e) {
      setServerError(classifyError(e));
    }
  };
      ...
      <div className="flex items-center justify-center bg-paper px-4 py-12 sm:px-8">
        <div className="w-full max-w-sm animate-fade-up">
          ...
          <p className="mt-1.5 text-sm text-ink-500">
            Sign in with Google or your work credentials. First-time candidates are redirected to registration.
          </p>
          ...
          <div className="mt-6">
            <button
              ...
              className="flex w-full items-center justify-center gap-2.5 rounded-lg border border-line-strong bg-white px-4 py-2.5 text-[13.5px] font-semibold text-ink-700 shadow-sm transition-all hover:border-ink-900/15 hover:bg-ink-900/[0.02] hover:shadow active:bg-ink-900/[0.04]"
          ...
          <div className="my-5 flex items-center gap-3">
            ...
            <span className="font-mono text-[10px] font-semibold uppercase tracking-[0.18em] text-ink-400">or sign in with email</span>
            <span className="h-px flex-1 bg-line-strong" />
          </div>
